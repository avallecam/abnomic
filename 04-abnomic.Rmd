...
microarray_proteins %>% export2xls("table/04-tab01-microarray_proteins.xls")
```
...
rho_pvalue
rho_pvalue %>% writexl::write_xlsx("table/04-tab02-validation_correlation.xlsx")
...
td %>% 
  ...
  ggrepel::geom_text_repel(data = td %>% 
                             ...
                             inner_join(all, by="Gene.ID") %>% 
                             inner_join(lg %>% dplyr::rename(seg.num=n), 
                                        by="Gene.ID")  %>% 
