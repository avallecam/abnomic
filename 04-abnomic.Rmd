...
ebaxx %>%
  ...
  ylab("Ab reactivity to Purified Antigen") + 
  xlab("Ab reactivity to IVTT Antigen")
...
severe_plot02_b <- td %>% 
  ...
  geom_vline(aes(xintercept=-1), lty=3) +
  geom_vline(aes(xintercept=1), lty=3) #+
  # ylim(0,3) # used for the final image
  #geom_vline(aes(xintercept=-1), lty=3) +
  # coord_cartesian(xlim = c(-2, 2), ylim = c(0,15)) 
# severe_plot02_b
...
antigen_plot_a <- rbind(a#,b
  ...
  ylab("Total SNPs (length-normalized)")+
  xlab("NonSyn/Syn SNP ratio (log2-transformed)")+
  labs(title="Genetic variability of antigens",#SNP space
       #subtitle="Among all strains",
...
antigen_plot_b <- rna_dt_subset %>% 
  ...
  # viridis::scale_color_viridis(#discrete = T,
  #                              option = "inferno") +
  coord_cartesian(xlim = c(min(rna_dt$rna_t), max(rna_dt$rna_t) + 15)) +
  facet_grid(.~subset) +
  ...
  xlab("Time (hours)") +
  ylab("log2 FPKM") +
  labs(title="Transcription profile of antigens during intraerythrocytic cycle",
       caption = "\n Data from Zhu et al. 2016") +
  #coord_trans(y = "log10")
