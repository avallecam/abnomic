...
pareto_pct <- pareto_table %>% filter(cut_median_value!="no") %>% pull(pct) %>% format(digits=2)
pareto_n <- pareto_table %>% filter(cut_median_value!="no") %>% pull(n)
...
```{r}
set_non_parametric <- TRUE #if change here, then next chunk too
```
...
td %>% 
  ...
  select(-Species,-logFC,-CI.L,-CI.R,-t,
         ...
         -Product.Description,-Gene.Name) %>% 
  # view()
  writexl::write_xlsx(paste0("table/04-tab03-severe_compare-pvalue_results-np_",
...
```{r}
sev_t %>% 
  select(-p.order,-Species,-seg.num) %>% 
  mutate(product_description = case_when(
    is.na(Gene.Name) ~ Product.Description,
    !is.na(Gene.Name) ~ str_c(Product.Description," (",Gene.Name,")")
  )) %>% 
...
eset_clean_diff_top_sub %>% 
  ...
  labs(y = NULL, x=NULL,
       caption = "*BH adjusted p-values between 0.968-0.690") + 
...
eset_clean_diff_top_sub %>%
  ...
  xlab("") +
  labs(caption = "*BH adjusted p-values between 0.968-0.690") +
  coord_flip()
...
ggsave("figure/04-fig09-severe_compare-heatmap-nocluster-ggplot-severe.png",
       height = 2.25,width = 6,dpi = "retina")
```
...
eset_clean_diff_top_sub %>% 
  ...
  ylab("Antigen reactivity") +
  xlab("") +
  labs(caption="*BH adjusted p-value <0.05 and log2 fold-change >1") +
  coord_flip()
  ...
  ggrepel::geom_text_repel(
    data = rbind(a,b
      ...
      dplyr::mutate(
        Gene.Name=ifelse(Gene.Name=="na",Gene.ID,Gene.Name),
        Gene.Name=ifelse(
          ID=="PVX_099980_1o1_S2.504","MSP1.S2",Gene.Name),
        Gene.Name=ifelse(
          ID=="PVX_099980_1o1_S1.793","MSP1.S1",Gene.Name)
        ) %>% 
...
ggsave("figure/04-fig11-severe_compare-transcription-heatmap.png",
       height = 4,width = 5.25,dpi = "retina")
...
ggsave("figure/04-fig12-severe_compare-antigen_mixtureplot.png",
...
antigen_plot_c <- rna_dt_subset_exposure %>% 
  ...
  labs(x="Time", y=NULL,
       title="Intraerythrocytic cycle Transcription profile",
       # subtitle = "Data from Zhu et al. 2016",
       caption = "FPKM = Fragments Per Kilobase Million\n*BH adjusted p-value <0.05 and log2 fold-change >1") +
  theme(
    axis.text.y = element_text(size=11),
    axis.text.x = element_text(size=12),
    strip.background = element_rect(
