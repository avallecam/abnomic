...
rbind(a,b,c,d) %>% 
  ...
  labs(
    ...
    caption="*Samples by Severe Malaria Study and Probes by P. vivax antigens") +
  theme(
...
eset_clean_diff_top_sub <- eset_clean_diff_top %>% 
  select(id.name,sample,value,sev_WHO,group)
...
eset_clean_diff_top_sub <- eset_clean_diff_top %>% 
  ...
  select(id.name,sample,value,sev_WHO=episodio_previo,group)
...
eset_clean_diff_top_sub %>% 
  ...
  labs(y = NULL, x=NULL,
       caption="*adjusted p-value <0.05 and log2 fold-change >1") + 
  theme(axis.text.x = element_text(angle = 90, 
                                   vjust = 0.5, 
                                   hjust=1,
                                   size=5),
...
eset_clean_diff_top %>%
  ...
  xlab("") +
  coord_flip()
ggsave("figure/04-fig23-exposure_compare-boxplot_highrve.png",
       height = 3,width = 6,dpi = "retina")
```
...
antigen_plot_c <- rna_dt_subset %>% 
  ...
  labs(x="Time", y=NULL,
       title="Transcription profile during intraerythrocytic cycle",
