...
feat <- raw %>% 
  ...
  #select(ID,1:10) %>% #head()
  select(ID,Description,Species) %>% #head()
  as.data.frame() %>% 
...
v11 <- EBA175 %>% 
  ...
  labs(title="Test validity: PfEBA175", 
       subtitle= paste0("S=",f$statistic,", rho=",f$estimate,", P=",f$p.value),
       caption=paste0("method: ",f$method)) #+ theme_classic()
  

v32 <- MSP2 %>% 
  ggplot(aes(x=MSP2_0.1.891,y=PF3D7_0206800.65)) + 
  geom_point() + #geom_smooth(method = "lm") +
  labs(title="Test validity: PfMSP2", 
       subtitle= paste0("S=",g$statistic,", rho=",g$estimate,", P=",g$p.value),
       caption=paste0("method: ",g$method)) #+ theme_classic()

v41 <- CSP %>% 
  ggplot(aes(x=PfCSP_0.3.894,y=PF3D7_0304600.932)) + 
  geom_point() + #geom_smooth(method = "lm") +
  labs(title="Test validity: PfCSP", 
       subtitle= paste0("S=",h$statistic,", rho=",h$estimate,", P=",h$p.value),
       caption=paste0("method: ",h$method)) #+ theme_classic()
...
__RANGO__ de valores `rho`: __`r min(v$rho)` - `r max(v$rho)`__
...
td <- topTable(eb, coef = 2,               # default: slope
  ...
  mutate(significance=if_else(adj.P.Val<0.05,
                              "adj.P.Val<0.05","reactive")) %>% #dplyr::count(significance)
...
```{r, fig.height=6, fig.width=5.4}
td %>% 
  mutate(unadj_significance=if_else(P.Value<0.05,
                              "P.Value<0.05","not sig.")) %>% #dplyr::count(significance)
  ggplot(aes(logFC,-log10(P.Value))) +
  geom_point(aes(colour=unadj_significance)) + scale_color_manual(values=c("black","red")) +
  ...
  ggrepel::geom_text_repel(data = td %>% 
                             ...
                             #top_n(10,no_severo)
                           ,
                           aes(label=Gene.ID), force = 6,
                           box.padding = unit(1, "lines"),
                           point.padding = unit(.5, "lines")
                           ) +
  labs(title="No_Severo vs Severo",
...
td %>% 
  ...
  arrange(P.Value) %>% 
  #arrange(desc(no_severo)) %>% 
  select(Gene.ID,Description,logFC,severo,no_severo,t,P.Value,adj.P.Val,p.order) # %>% slice(1:10)
```
...
td %>% 
  ...
  select(Gene.ID,Description,AveExpr,p.order) %>% slice(1:10)
```
...
pData(eset) <- biobroom::tidy.ExpressionSet(eset,addPheno = TRUE) %>% 
  ...
  full_join(pData(eset) %>% 
            ...
            by="sample") %>% 
  select(-Study,-Group,-episodio_previo,-edad_CAT, -expo_CAT) %>% 
...
biobroom::tidy.ExpressionSet(eset,addPheno = TRUE) %>% #count(gene)
  ...
  filter(group!="NA") %>% 
  ggplot(aes(reorder(Gene.ID,no_severo),value,fill=sev_WHO))+
  geom_boxplot()+
  #geom_point() +
  ...
  labs(title="Highly and Differentially reactive antigens",
       subtitle="All sorted w.r.t. no_severo mean Ab reactivity",
       caption="*filtered by non-adjust P.Value<0.05")+
  coord_flip() # ORDENAR eje de proteínas!!!!
```

#### 5.2 previous episodes
...
td %>% 
  ...
  ggrepel::geom_text_repel(data = td %>% 
                             ...
                             #top_n(10,con)
                           ,
                           aes(label=Gene.ID), force = 15,
                           box.padding = unit(1, "lines"),
                           point.padding = unit(1, "lines")
                           ) +
  labs(title="Con vs Sin episodio previo",
       subtitle="Tamaño del efecto y significancia estadística",
       caption="annotated features filtered by\n(adj.P.Val<0.05 & logFC>1.5 & con>1)")+
...
td %>% 
  ...
  arrange(desc(con)) %>% 
  select(Gene.ID,Description,logFC,con,sin,P.Value,adj.P.Val,p.order) # %>% slice(1:10)
```
...
pData(eset) <- biobroom::tidy.ExpressionSet(eset,addPheno = TRUE) %>% 
  ...
  full_join(pData(eset) %>% 
              ...
              dplyr::rename(sample=rowname),
            by="sample") %>% 
  select(-Study,-Group,-sev_WHO,-edad_CAT, -expo_CAT) %>% 
...
```{r, fig.width=6,fig.height=4.5, echo=TRUE}
eset <- eset.VIVAX.SEV.FILTER
...
biobroom::tidy.ExpressionSet(eset,addPheno = TRUE) %>% #count(gene)
  ...
  filter(group!="NA") %>% 
  ggplot(aes(reorder(Gene.ID,con),value,fill=episodio_previo))+
  geom_boxplot()+
  #geom_point() +
  facet_grid(group~.,scales = "free", space = "free") +#
  ylab("log2-transformed normalized MFI")+
  xlab("antigens")+
  labs(title="Highly and Differentially reactive antigens",
       subtitle="All sorted w.r.t. con mean Ab reactivity",
       caption="*filtered by adj.P.Val<0.05 and \n (logFC>1 & con>1) | (logFC>0.9 & con>2)")+
  coord_flip() # ORDENAR eje de proteínas!!!!
```
...
r <- fin %>% 
  ...
  labs(title="Breadth of Ab response", 
       ...
       caption=paste0("method:",j$method)) #+ theme_classic()
...
s <- fin %>% 
  ...
  labs(title="Intensity of Ab response", 
       ...
       caption=paste0("method:",k$method)) #+ theme_classic()
...
r <- fin %>%
  ...
  geom_dotplot(#aes(fill=sev_WHO), 
               binaxis='y', stackdir='center', 
               dotsize=1, position=position_dodge(0.8)) +
  labs(title="Breadth of Ab response", subtitle="per age category") #+ theme_classic()
...
s <- fin %>% 
  ...
  geom_dotplot(#aes(fill=sev_WHO), 
               binaxis='y', stackdir='center', 
               dotsize=1, position=position_dodge(0.8)) +
  labs(title="Intensity of Ab response", subtitle="per age category") #+ theme_classic()
...
#               dotsize=1, position=position_dodge(0.8)) +
#  labs(title="Breadth of Ab response", 
#       subtitle="per previous episode category") #+ theme_classic()
#
...
t <- fin %>% 
  ...
  geom_dotplot(aes(fill=episodio_previo), 
               binaxis='y', stackdir='center', 
               dotsize=1, position=position_dodge(0.8)) +
  labs(title="Breadth of Ab response", subtitle="per previous episode category") #+ theme_classic()
...
u <- fin %>% 
  ...
  geom_dotplot(aes(fill=episodio_previo), 
               binaxis='y', stackdir='center', 
               dotsize=1, position=position_dodge(0.8)) +
  labs(title="Intensity of Ab response", subtitle="per previous episode category") #+ theme_classic()
