...
rbind(a,b,c,d) %>% 
  ...
  labs(title="Data distribution per pre-processing step",
       caption="*Samples by Severe Malaria Study\nProbes by P. vivax antigens") +
...
r <- fin %>% 
  ...
  xlab("WHO criteria") + ylab("Number Reactive") +
...
s <- fin %>% 
  ...
  labs(title="Intensity of Ab response", 
       subtitle= paste0("t=",k$statistic %>% format(digits=2),
...
```{r}
exp_da %>% inner_join(sev_d, by="Gene.ID") %>% 
  select(Gene.ID,Product.Description.x,Gene.Name.x,con,sin,adj.P.Val.x,p.order.x,severo,no_severo,P.Value.y,adj.P.Val.y,p.order.y)
...
rbind(a,b,c,d) %>% 
  ...
  #guides(fill = guide_legend(title.position = "left")) +
  theme(legend.position=c(-.28, .18),#"bottom"
        legend.margin = margin(0,0,0,0),
        axis.title.y=element_blank()
        #legend.text = element_text(size = 8),
        #legend.title = element_text(size = 8)
  ...
  #ylab("log2-transformed normalized MFI")+
  #xlab(label = "")+
  labs(title="Protein targeting and localization per subset") +
...
rbind(a,b,c,d) %>% 
  ...
  facet_grid(.~summary.db, scales = "free" # CON O SIN scala
             #space = "free"
             ) +#
  coord_flip() + theme_bw() +
  scale_fill_discrete(name="Protein",
                      labels=c("with annotation", "hypothetical")) +
  theme(legend.position=c(-.38, 0.08),#"bottom"
        legend.margin = margin(0,0,0,0),
        axis.title.y=element_blank()
        #legend.text = element_text(size = 8),
        #legend.title = element_text(size = 8)
        ) +
  labs(title="Gene Ontology: Cellular Component prediction per subset") +
  theme(strip.background = element_rect(colour = "black"#, fill = "white"
                                        ),
        strip.text.x = element_text(colour = "black"#,size = 10#, face = "bold"
                                    ))
...
rbind(a,#b,
  ...
  scale_fill_discrete(name="Protein",
                      labels=c("with annotation", "hypothetical")) +
  theme(legend.position=c(-.5, 0.07),#"bottom"
        legend.margin = margin(0,0,0,0),
        axis.title.y=element_blank()
        #legend.text = element_text(size = 8),
        #legend.title = element_text(size = 8)
...
```{r,fig.height=8.5,fig.width=10, eval=FALSE}
go_ord <- rbind(all_go,
      #a %>% select(-ID),
      c %>% select(-ID),
      d %>% select(-ID)
...
rbind(all_go,
  ...
  scale_fill_discrete(name="Protein",
                      labels=c("with annotation", "hypothetical")) +
  theme(legend.position=c(-.48, 0.04),#"bottom"
        legend.margin = margin(0,0,0,0),
        axis.title.y=element_blank()
        #legend.text = element_text(size = 8),
        #legend.title = element_text(size = 8)
...
rna_dtm <- rna_dt %>% 
  ...
  #filter(rna_s=="s1") %>% 
  #filter(subset=="diff") %>%
  select(ID,id.name,rna_t,fpkm) %>% 
  #dplyr::mutate(fpkm=log2(fpkm)) %>%
  #filter(fpkm!="-Inf") %>% 
  reshape2::acast(id.name ~ rna_t,
                  value.var = "fpkm") #%>% class()
aheatmap(rna_dtm, #info = TRUE,
         #main = "Transcription profile of intraerythrocytic cycle (Zhu et al. 2016)",
         Rowv = TRUE, Colv = NA#, 
         #annCol = pData(eset), 
         #layout = "_"
         )
