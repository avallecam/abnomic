...
```{r mean-sd-plot, fig.height=4, fig.width=10, eval=FALSE, echo=FALSE}
...
```{r messy-tidy, eval=FALSE, echo=FALSE}
#### messy BEGINNER script
...
```{r messy-log-transformation, eval=FALSE, echo=FALSE}
...
xxx<-anti.raw_data/median.ctrl.raw_data.mat<0 # [06jul2016] rpta: only 4
sum(xxx)
...
```{r median-centering, eval=FALSE, echo=FALSE}
...
#**(I/O) OFF**
normalized.log2.NA.norm.ANTIGENS <- log2.NA.norm.ANTIGENS # IF READ, MEDIAN CENTERING IS OFF!!
```
...
norm %>% as.data.frame() %>% 
  ...
  ggplot(aes(sample_name,expression)) +
  geom_boxplot() +
...
norx <- norm %>% as.data.frame() %>% 
  ...
  mutate(expression= ifelse(expression < -5, NA, expression)) %>% 
...
```{r messy-outlier, eval=FALSE, echo=FALSE}
...
```{r messy-eset-integration, eval=FALSE, echo=FALSE}
...
#For a complete overview of this procedure, 
#click [here](kasperdanielhansen.github.io/genbioconductor/pdf/BiocIntro_ExpressionSet_Overview.pdf).
#
#For a single capture of the data structure, 
#take a look at this [image](https://i.ytimg.com/vi/5EAOwLDD6Wo/maxresdefault.jpg).
...
pheno.t <- read.table("data-raw/samples.csv",row.names=2,header=TRUE, sep=","
                    #, 
...
##
eset.VIVAX.PQ<-eset[featureData(eset)$Species=="Pv",phenoData(eset)$Study=="Primaquine"]
#eset.VIVAX.PQ
#
eset.FALCIP.PQ<-eset[featureData(eset)$Species=="Pf",phenoData(eset)$Study=="Primaquine"]
#eset.FALCIP.PQ
##
eset.VIVAX.SEV<-eset[featureData(eset)$Species=="Pv",phenoData(eset)$Study!="Primaquine"]
eset.VIVAX.SEV
#
eset.FALCIP.SEV<-eset[featureData(eset)$Species=="Pf",phenoData(eset)$Study!="Primaquine"]
#eset.FALCIP.SEV
##
```
...
a <- anti %>% as.data.frame() %>% 
  ...
  geom_histogram() +
  labs(title="RAW")
b <- norx %>% as.data.frame() %>% 
  gather(sample_name,expression) %>% 
  ggplot(aes(expression)) +
  geom_histogram() +
  labs(title="NORMALIZED+TRANSFORMED")
c <- exprs(eset.VIVAX.SEV) %>% as.data.frame() %>% 
  gather(sample_name,expression) %>% 
  ggplot(aes(expression)) +
  geom_histogram() +
  labs(title="SEVERE VIVAX DATA")
d <- exprs(eset.VIVAX.SEV.FILTER) %>% as.data.frame() %>% 
  gather(sample_name,expression) %>% 
  ggplot(aes(expression)) +
  geom_histogram() +
  labs(title="SEVERE VIVAX FILTERED DATA")
...
```{r, fig.height=15, fig.width=20, eval=FALSE, echo=FALSE}
## whole dataset
...
#
plotDensities(anti, 
              legend = FALSE, main = "Raw Intensity")
plotDensities(anti/ctrm, 
              legend = FALSE, main = "Normalized Intensity")#log2.NA.norm.ANTIGENS
plotDensities(norx, 
              legend = FALSE, main = "Transformed Intensity")
plotDensities(exprs(eset.FILTER), 
              legend = FALSE, main = "Filtered Intensity")
#
hist(anti,
     breaks=600, col = "black", main = NULL, ylim = c(0,2500),
     ylab="Raw Intensity")
hist(anti/ctrm,
     breaks=600, col = "black", main = NULL, ylim = c(0,2500),
...
hist(norx,
     breaks=600, col = "black", main = NULL, ylim = c(0,2500),
     ylab="Transformed Intensity")#log2.NA.norm.ANTIGENS
hist(exprs(eset.FILTER),
     breaks=600, col = "black", main = NULL, ylim = c(0,2500),
     ylab="Filtered Intensity")           # THIS IS IT!!
#
boxplot(anti,
        col=colours,pch=20, las=2, 
        ylab="Raw Intensity")
boxplot(anti/ctrm,
        col=colours,      #sweep --> MEDIAN CENTERING
        pch=20, ylim=c(-3,6.3),
        las=2, 
        ylab="Normalized Intensity") # ¿check this? SINCE output of negative values turns to NA, median gets NULL
boxplot(norx,
        col=colours,                 #JUST log2 NORMALIZATION #log2.NA.norm.ANTIGENS
        pch=20, #ylim=c(-3,6.3),
        las=2, 
        ylab="Transformed Intensity")
boxplot(exprs(eset.FILTER),
        col=colours,
        pch=20, ylim=c(-3,6.3),
        las=2, 
        ylab="Filtered Intensity")                             # REQUIRED!!
...
```{r, fig.height=10, fig.width=20, eval=FALSE, echo=FALSE}
## Pq & Sev
# [ADDED 28jul2016]
...
SV_main<-read_stata("data-raw/malaria severa 28Nov2014_Ed.dta")             #----* WITHOUT LABELS
#
...
write.csv(SV_main, file="data/SEVERE_28nov2014.csv")
#
tabAll <- read.table("data/SEVERE_28nov2014.csv", row.names=2, header=TRUE, sep=",")
##
...
###################
write.csv(tabSELECT, file="data/SEVERE_28nov2014_SELECTEDcovariates.csv")

sevData<-read.table("data/SEVERE_28nov2014_SELECTEDcovariates.csv",row.names=1,header=TRUE, sep=",", 
                        colClasses = c(rep("factor",5),"numeric","factor","numeric","numeric",rep("factor",5),"numeric","numeric",
                                       rep("factor",5),rep("numeric",9),"factor","numeric",rep("factor",2),rep("numeric",2)))
...
```{r, eval=FALSE, message=FALSE, fig.align="center", fig.width=8, fig.height=4}
a1 <- ggplot(sampleSEV, aes(x=malaria_pasado_num)) + 
  geom_histogram(alpha=.5) + 
  labs(title="Histograma de Episodios previos") #+
  #geom_density() #+
...
```{r, eval=FALSE, message=FALSE, fig.align="center", fig.width=12, fig.height=4}
...
```{r, eval=FALSE, message=FALSE, fig.align="center", fig.width=12, fig.height=4}
a <- ggplot(sampleSEV, aes(x=malaria_pasado_num, fill=casecontrol)) + 
  geom_histogram(
    #aes(y=..density..), 
                 #breaks=seq(0, 85, by = 5), 
                 alpha=.5 ,position = "identity") + 
...
b <- ggplot(sampleSEV, aes(x=malaria_pasado_num, fill=con_exp)) + 
  geom_histogram(
    #aes(y=..density..), 
                 #breaks=seq(0, 85, by = 5), 
                 alpha=.5 ,position = "identity") + 
...
#### Clinical data
```{r, eval=FALSE, warning=FALSE, fig.height=5}
...
```{r, eval=FALSE, warning=FALSE,  fig.width=8, fig.height=4}
par(mfrow=c(1,2))
...
```{r, eval=FALSE, warning=FALSE,  fig.width=10, fig.height=10}
par(mfrow=c(3,3))
...
According to the most recent WHO report[@WHO2014severe], for epidemiological and research purposes, **Severe Malaria** is defined as one or more of the following, occurring in the absence of an identified alternative cause, and in the presence of asexual parasitaemia:

```{r, eval=FALSE, warning=FALSE, fig.height=9, fig.width=10}
...
sampleSEVcovariates <- pbc[,c("casecontrol", "CumSumSevere7", "CumSumSevere10", "con_exp","expo_CAT", "malaria_pasado_num", "edad_CAT", "edad", "parasitemia")]
...
```{r, eval=FALSE, fig.align="center", fig.height=6, fig.width=10, warning=FALSE}
...
```{r, eval=FALSE, fig.align="center", fig.height=8, fig.width=10, warning=FALSE}
#str(sampleSEV)
...
```{r, eval=FALSE, fig.height=3, fig.width=9, fig.align='center'}
par(mfrow=c(1,3))
...
```{r, eval=FALSE, fig.height=6, fig.width=6, fig.align='center', warning=FALSE, message=FALSE}
#library(ggplot2)
#library(Rmisc)
...
```{r, eval=FALSE, echo=FALSE}
# CONTROL: E (NOT-SEVERE) -> CASE: D (severe)

#sevdb <- readRDS("data/02-sevrcov.rds") # previous: nov2014.dta ---------------- perfect case definition
#sevdb <- readRDS("data/01-sevrcov.rds") # update: sep2016.dta ------------------ 9 updates
sevdb <- readRDS("data/03-sevrcov.rds") # merged: nov2014.dta + sep2016.dta ---- 23 updates

pData(normal.log2.raw.eSet.VIVAX.SEV.FILTER) %>% as_tibble() %>% #glimpse() #%>% class()
  select(Subject.ID, Study, Group) %>% #glimpse()
  dplyr::rename(codigo= Subject.ID) %>% 
  dplyr::mutate(codigo= as.character(codigo)) %>% #glimpse()
  full_join(sevdb %>% 
              select(codigo, sev_WHO, malaria_pasado),
            by="codigo") %>% #glimpse()
  group_by(Group, sev_WHO#, malaria_pasado
           ) %>% dplyr::count()

#sevdb %>% #glimpse()
#  select(codigo, sev_WHO, malaria_pasado) %>% glimpse()
```
...
##### By Age category
*potentially GENERAL*
```{r, eval=FALSE, echo=FALSE, fig.height=8, fig.width=8, fig.align="center"}
#
eset<-normal.log2.raw.eSet.VIVAX.SEV.FILTER
eset$Group <- factor(eset$Group)
#
...
```{r, fig.height=4, fig.width=9, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE, echo=FALSE}
...
```{r, fig.height=4, fig.width=14, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE, echo=FALSE}
#library(ggplot2)
#library(Rmisc)
...
```{r, fig.height=3, fig.width=8, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE, echo=FALSE}
#library(ggplot2)
#library(Rmisc)
...
```{r, fig.height=8, fig.width=8, fig.align="center", eval=FALSE, eval=FALSE, echo=FALSE}
##### THIS IS THE RIGHT HEATMAP (with unordered groups)
...
### Technical plots
#### Backgroud effect
```{r, fig.height=8, fig.width=8, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE, echo=FALSE}
...
```{r, fig.width=14, fig.height=7, fig.align='center', eval=FALSE, echo=FALSE}
par(mfrow=c(1,2))
...
aheatmap(rbind(rawhm,ctrlhm), Rowv = NA, Colv = NA, main = "Raw Intensities")
aheatmap(exprs(eset), Rowv = NA, Colv = NA, main = "Transformed and Normalized Intensities")
```
...
m <- anti %>% as.data.frame() %>% 
  ...
  geom_point() +
  geom_smooth() +
  labs(title="Raw mean-variance dependence")
...
n <- norx %>% as.data.frame() %>% 
  ...
  geom_point() +
  geom_smooth() +
  labs(title="Normalized mean-variance dependence")

Rmisc::multiplot(m,n,cols = 2)
```

```{r, fig.height=4, fig.width=8, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE, echo=FALSE}
#
RAW_data <- anti.raw_data[rownames(row.mean.m_dataXX),]
...
# PF3D7_0930300_s1.67   falciparum      IVTT.AG PF3D7_0930300   merozoite surface protein 1 (MSP1)
MSP1s1 <- as.data.frame(cbind(as.matrix(anti["PF3D7_0930300_s1.67",]), as.matrix(pure["MSP1, 0.3mg/mL.601",]), as.matrix(pure["MSP1, 0.1mg/mL.602",])))
colnames(MSP1s1) <- c("PF3D7_0930300_s1.67", "MSP1_0.3.601", "MSP1_0.1.602")

# PF3D7_0930300_s2.941  falciparum      IVTT.AG PF3D7_0930300   merozoite surface protein 1 (MSP1)
MSP1s2 <- as.data.frame(cbind(as.matrix(anti["PF3D7_0930300_s2.941",]), as.matrix(pure["MSP1, 0.3mg/mL.601",]), as.matrix(pure["MSP1, 0.1mg/mL.602",])))
colnames(MSP1s2) <- c("PF3D7_0930300_s2.941", "MSP1_0.3.601", "MSP1_0.1.602")

# PF3D7_0206800.65      falciparum      IVTT.AG PF3D7_0206800   merozoite surface protein 2 (MSP2)
MSP2 <- as.data.frame(cbind(as.matrix(anti["PF3D7_0206800.65",]), as.matrix(pure["MSP2, 0.3mg/mL.890",]), as.matrix(pure["MSP2, 0.1mg/mL.891",])))
colnames(MSP2) <- c("PF3D7_0206800.65", "MSP2_0.3.890", "MSP2_0.1.891")

# PF3D7_0304600.932     falciparum      IVTT.AG PF3D7_0304600   circumsporozoite (CS) protein (CSP)
CSP <- as.data.frame(cbind(as.matrix(anti["PF3D7_0304600.932",]), as.matrix(pure["Pf CSP, 0.3mg/mL.894",]), as.matrix(pure["Pf CSP, 0.1mg/mL.895",])))
colnames(CSP) <- c("PF3D7_0304600.932", "PfCSP_0.3.894", "PfCSP_0.1.895")

# VIVAX AMA1
PvAMA1 <- as.data.frame(cbind(as.matrix(pure["Pvivax AMA1, 0.3mg/mL.603",]), as.matrix(pure["Pvivax AMA1, 0.1mg/mL.604",]), as.matrix(pure["Pvivax AMA1 Eoto monomer prep2, 0.3mg/mL.892",]), as.matrix(pure["Pvivax AMA1 Eoto monomer prep2, 0.1mg/mL.893",])))
...
cor.test(~ PF3D7_0731500_e1s2.361 + EBA175_0.3.23, EBA175, method = "spearman")

#library(PerformanceAnalytics)
chart.Correlation(EBA175, method = "spearman")
chart.Correlation(MSP2, method = "spearman")
chart.Correlation(CSP, method = "spearman")
chart.Correlation(PvAMA1, method = "spearman")
...
```{r, warning=FALSE, fig.align="center", fig.height=6, fig.width=10}
chart.Correlation(MSP1s1, method = "spearman")
chart.Correlation(MSP1s2, method = "spearman")
```

##### **MSP1_S1 distribution**

- necesita refinar!

```{r, fig.height=8, fig.width=8, fig.align="center", message=FALSE, eval=FALSE, echo=FALSE}
# PF3D7_0620400.974	falciparum	IVTT.AG	PF3D7_0620400	merozoite surface protein 10 (MSP10)
# PVX_114145_1o1.272	vivax	IVTT.AG	PVX_114145	Merozoite surface protein 10, MSP10

# PVX_099980_1o1_S1.793	IVTT.AG	Pv	PVX_099980	major blood-stage surface antigen Pv200 [PvMSP1-S1]
# PVX_099980_1o1_S2.504	IVTT.AG	Pv	PVX_099980	major blood-stage surface antigen Pv200 [PvMSP1-S2]
# PF3D7_0930300_s1.67	IVTT.AG	Pf	PF3D7_0930300	merozoite surface protein 1 (MSP1) [PfMSP1-S1]
# PF3D7_0930300_s2.941	IVTT.AG	Pf	PF3D7_0930300	merozoite surface protein 1 (MSP1) [PfMSP1-S2]
# MSP1, 0.3mg/mL.601	PURIFIED.PROTEIN	Pf	NA	MSP1, 0.3mg/mL
# MSP1, 0.1mg/mL.602	PURIFIED.PROTEIN	Pf	NA	MSP1, 0.1mg/mL


MSP1_S1 <- as.data.frame(cbind(as.matrix(exprs(normal.log2.raw.eSet)["PF3D7_0930300_s1.67",]),
                             as.matrix(exprs(normal.log2.raw.eSet)["PVX_099980_1o1_S1.793",])))
MSP1_S1 <- cbind(MSP1_S1,rownames(MSP1_S1),pData(normal.log2.raw.eSet)[,"Study"])
colnames(MSP1_S1) <- c("PfMSP1_S1", "PvMSP1_S1", "sample.ID", "Study")

MSP1_S1$Study <- factor(MSP1_S1$Study)

#length(rownames(MSP1_S1))
#phenoData(normal.log2.raw.eSet)
#pData(normal.log2.raw.eSet)
#varLabels(normal.log2.raw.eSet)

MSP1_S1.x <- MSP1_S1 %>%
  gather(specie, intensity, PfMSP1_S1:PvMSP1_S1) %>%
  ...
  #geom_density(alpha=.3) +
  labs(title="Normalized intensity against MSP1_S1 \n per specie")
MSP1_S1.xd <- MSP1_S1 %>%
  gather(specie, intensity, PfMSP1_S1:PvMSP1_S1) %>%
  mutate(specie=factor(specie)) %>%
  #filter(con_exp=="CON") %>%
...
#
MSP1_S1 <- subset(MSP1_S1, Study!="Primaquine")
MSP1_S1 <- cbind(MSP1_S1, pData(normal.log2.raw.eSet.VIVAX.SEV.FILTER)[,c("casecontrol","con_exp")])
#levels(MSP1_S1$con_exp) <- c("CON","SIN")
#MSP1_S1$Study <- factor(MSP1_S1$Study)
...
MSP1_S1.w <- MSP1_S1 %>%
  ...
  mutate(specie=factor(specie)) %>%
  #filter(Study!="Primaquine") %>%
  filter(specie=="PvMSP1_S1") %>%
  #mutate(feature=factor(feature)) %>%
  ggplot(aes(x=intensity,fill=con_exp)) + 
  geom_histogram(alpha=.5 ,position = "identity") + 
  #geom_density(alpha=.3) +
  labs(title="Normalized intensity \n only Severe study and PvMSP1_S1")
MSP1_S1.wd <- MSP1_S1 %>%
  gather(specie, intensity, PfMSP1_S1:PvMSP1_S1) %>%
  mutate(specie=factor(specie)) %>%
  #filter(Study!="Primaquine") %>%
...
multiplot(MSP1_S1.x, MSP1_S1.z, MSP1_S1.w, #MSP1_S1.y,
          MSP1_S1.xd, MSP1_S1.zd, MSP1_S1.wd, #MSP1_S1.yd,
          cols = 2)
```

##### **MSP1_S2 distribution**
...
MSP1_S2 <- as.data.frame(cbind(as.matrix(exprs(normal.log2.raw.eSet)["PF3D7_0930300_s2.941",]),
                               as.matrix(exprs(normal.log2.raw.eSet)["PVX_099980_1o1_S2.504",])))
MSP1_S2 <- cbind(MSP1_S2,rownames(MSP1_S2),pData(normal.log2.raw.eSet)[,"Study"])
colnames(MSP1_S2) <- c("PfMSP1_S2", "PvMSP1_S2", "sample.ID", "Study")

MSP1_S2$Study <- factor(MSP1_S2$Study)

#length(rownames(MSP1_S2))
#phenoData(normal.log2.raw.eSet)
#pData(normal.log2.raw.eSet)
#varLabels(normal.log2.raw.eSet)

MSP1_S2.x <- MSP1_S2 %>%
  gather(specie, intensity, PfMSP1_S2:PvMSP1_S2) %>%
  mutate(specie=factor(specie)) %>%
  #filter(con_exp=="CON") %>%
  #mutate(feature=factor(feature)) %>%
  ggplot(aes(x=intensity,fill=specie)) + 
  geom_histogram(alpha=.5 ,position = "identity") + 
  #geom_density(alpha=.3) +
  labs(title="Normalized intensity against MSP1_S2 \n per specie")
MSP1_S2.xd <- MSP1_S2 %>%
  gather(specie, intensity, PfMSP1_S2:PvMSP1_S2) %>%
  mutate(specie=factor(specie)) %>%
  #filter(con_exp=="CON") %>%
...
#
MSP1_S2 <- subset(MSP1_S2, Study!="Primaquine")
MSP1_S2 <- cbind(MSP1_S2, pData(normal.log2.raw.eSet.VIVAX.SEV.FILTER)[,c("casecontrol","con_exp")])
#levels(MSP1_S2$con_exp) <- c("CON","SIN")
#MSP1_S2$Study <- factor(MSP1_S2$Study)
#varLabels(normal.log2.raw.eSet.VIVAX.SEV.FILTER) #,
#phenoData(normal.log2.raw.eSet.VIVAX.SEV.FILTER)
...
MSP1_S2.w <- MSP1_S2 %>%
  ...
  mutate(specie=factor(specie)) %>%
  #filter(Study!="Primaquine") %>%
  filter(specie=="PvMSP1_S2") %>%
  #mutate(feature=factor(feature)) %>%
  ggplot(aes(x=intensity,fill=con_exp)) + 
  geom_histogram(alpha=.5 ,position = "identity") + 
  #geom_density(alpha=.3) +
  labs(title="Normalized intensity \n only Severe study and PvMSP1_S2")
MSP1_S2.wd <- MSP1_S2 %>%
  gather(specie, intensity, PfMSP1_S2:PvMSP1_S2) %>%
  mutate(specie=factor(specie)) %>%
  #filter(Study!="Primaquine") %>%
...
multiplot(MSP1_S2.x, MSP1_S2.z, MSP1_S2.w, #MSP1_S2.y,
          MSP1_S2.xd, MSP1_S2.zd, MSP1_S2.wd, #MSP1_S2.yd,
          cols = 2)
```
...
MSP10 <- as.data.frame(cbind(as.matrix(exprs(normal.log2.raw.eSet)["PF3D7_0620400.974",]),
                               as.matrix(exprs(normal.log2.raw.eSet)["PVX_114145_1o1.272",])))
...
MSP10.x <- MSP10 %>%
  ...
  geom_histogram(alpha=.5 ,position = "identity") + 
  #geom_density(alpha=.3) +
  labs(title="Normalized intensity against MSP10 \n per specie")
MSP10.xd <- MSP10 %>%
  gather(specie, intensity, PfMSP10:PvMSP10) %>%
  mutate(specie=factor(specie)) %>%
  #filter(con_exp=="CON") %>%
...
**Hypothesis Generation**, but not confirmation due to low statistical power.

###### Per age and exposure
```{r, fig.height=6, fig.width=7, fig.align='center', warning=FALSE, message=FALSE}
corrXX <- subset(pData(esetEXTRA),select= c(casecontrol,con_exp,expo_CAT,edad_CAT:mean.intensity))
#kable(head(corrX[order(corrX$edad, decreasing = TRUE),]), align = "c")

#corrX
EdadBreadth1x <- ggplot(corrXX, aes(x=edad_CAT,y=breadth.percentage
                                  #, fill=casecontrol
                                  )) +
  geom_boxplot() + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1) + theme_bw()

#+ geom_point() + geom_smooth(alpha=.2) #+ ggtitle("xx") + geom_line() + # group=con_exp
...
EdadBreadth2x <- ggplot(corrXX, aes(x=edad_CAT,y=mean.intensity
                                  #, fill=casecontrol
                                  )) +
  geom_boxplot() + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1) + theme_bw()
...
EdadBreadth3x <- ggplot(corrXX, aes(x=expo_CAT,y=breadth.percentage
                                  #, fill=con_exp
                                  )) +
  geom_boxplot() + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1) + theme_bw()
...
# + geom_point() + geom_smooth(alpha=.2) #+ ggtitle("xx") + geom_line() + # group=con_exp
#ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=casecontrol)) + geom_point() + geom_smooth(alpha=.2,size=.1) #+ ggtitle("xx") + geom_line() + # group=con_exp
EdadBreadth4x <- ggplot(corrXX, aes(x=expo_CAT,y=mean.intensity
                                  #, fill=con_exp
                                  )) +
  geom_boxplot() + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1) + theme_bw()
...
multiplot(EdadBreadth1x,EdadBreadth3x,EdadBreadth2x,EdadBreadth4x,cols=2)
```

```{r,fig.height=6, fig.width=8, fig.align='center', eval=FALSE}
#corrXX

a <- ggplot(corrXX, aes(x=breadth.percentage#,y=
                                  , fill=edad_CAT
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) #+

b <- ggplot(corrXX, aes(x=breadth.percentage#,y=
                                  , fill=expo_CAT
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) #+

c <- ggplot(corrXX, aes(x=mean.intensity#,y=
                                  , fill=edad_CAT
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) #+

d <- ggplot(corrXX, aes(x=mean.intensity#,y=
                                  , fill=expo_CAT
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) #+
...
#corrX
EdadBreadth1i <- ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=casecontrol)) + 
  geom_point() + geom_smooth(alpha=.2)  + theme_bw()
#+ ggtitle("xx") + geom_line() + # group=con_exp
#ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=casecontrol)) + geom_point() + geom_smooth(alpha=.2,size=.1) #+ ggtitle("xx") + geom_line() + # group=con_exp
EdadBreadth2i <- ggplot(corrX, aes(x=edad,y=mean.intensity, colour=casecontrol)) + 
  geom_point() + geom_smooth(alpha=.2)  + theme_bw()
#+ ggtitle("xx") + geom_line() + # group=con_exp

EdadBreadth3i <- ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=con_exp)) + 
  geom_point() + geom_smooth(alpha=.2)  + theme_bw()
#+ ggtitle("xx") + geom_line() + # group=con_exp
#ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=casecontrol)) + geom_point() + geom_smooth(alpha=.2,size=.1) #+ ggtitle("xx") + geom_line() + # group=con_exp
EdadBreadth4i <- ggplot(corrX, aes(x=edad,y=mean.intensity, colour=con_exp)) + 
  geom_point() + geom_smooth(alpha=.2)  + theme_bw()
#+ ggtitle("xx") + geom_line() + # group=con_exp


multiplot(EdadBreadth1i,EdadBreadth3i,EdadBreadth2i,EdadBreadth4i,cols=2)
```
...
#corrX
EdadBreadth1 <- ggplot(corrX, aes(x=edad_CAT,y=breadth.percentage, fill=casecontrol)) +
  geom_boxplot(#aes(colour=casecontrol),
               position=position_dodge(0.8), outlier.color = NA) + 
  geom_dotplot(aes(fill=casecontrol), binaxis='y', stackdir='center', dotsize=1, position=position_dodge(0.8)) + theme_bw()
#scale_fill_grey()
#+ geom_point() + geom_smooth(alpha=.2) #+ ggtitle("xx") + geom_line() + # group=con_exp
#ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=casecontrol)) + geom_point() + geom_smooth(alpha=.2,size=.1) #+ ggtitle("xx") + geom_line() + # group=con_exp
EdadBreadth2 <- ggplot(corrX, aes(x=edad_CAT,y=mean.intensity, fill=casecontrol)) +
  geom_boxplot(#aes(colour=casecontrol),
               position=position_dodge(0.8), outlier.color = NA) + 
  geom_dotplot(aes(fill=casecontrol), binaxis='y', stackdir='center', dotsize=1, position=position_dodge(0.8)) + theme_bw()
# + geom_point() + geom_smooth(alpha=.2) #+ ggtitle("xx") + geom_line() + # group=con_exp

EdadBreadth3 <- ggplot(corrX, aes(x=edad_CAT,y=breadth.percentage, fill=con_exp)) +
  geom_boxplot(#aes(colour=con_exp),
               position=position_dodge(0.8), outlier.color = NA) + 
  geom_dotplot(aes(fill=con_exp), binaxis='y', stackdir='center', dotsize=1, position=position_dodge(0.8)) + theme_bw()
# + geom_point() + geom_smooth(alpha=.2) #+ ggtitle("xx") + geom_line() + # group=con_exp
#ggplot(corrX, aes(x=edad,y=breadth.percentage, colour=casecontrol)) + geom_point() + geom_smooth(alpha=.2,size=.1) #+ ggtitle("xx") + geom_line() + # group=con_exp
EdadBreadth4 <- ggplot(corrX, aes(x=edad_CAT,y=mean.intensity, fill=con_exp)) +
  geom_boxplot(#aes(colour=con_exp),
               position=position_dodge(0.8), outlier.color = NA) + 
  geom_dotplot(aes(fill=con_exp), binaxis='y', stackdir='center', dotsize=1, position=position_dodge(0.8)) + theme_bw()
# + geom_point() + geom_smooth(alpha=.2) #+ ggtitle("xx") + geom_line() + # group=con_exp
...
```
```{r,fig.height=4, fig.width=14, fig.align='center', eval=FALSE}
a <- ggplot(corrX, aes(x=breadth.percentage#,y=
                                  , fill=casecontrol
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) +
...
b <- ggplot(corrX, aes(x=breadth.percentage#,y=
                                  , fill=con_exp
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) +
...
c <- ggplot(corrX, aes(x=mean.intensity#,y=
                                  , fill=casecontrol
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) +
...
d <- ggplot(corrX, aes(x=mean.intensity#,y=
                                  , fill=con_exp
                                  )) +
  #geom_histogram(alpha=.5 ,position = "identity") +
  geom_density(alpha=.3) +
...
```{r, fig.height=4, fig.width=16, message=FALSE, eval=FALSE}
...
boxplot(breadth.percentage ~ Group, data= pData(eset), main= "Breadth of Response", ylab= "Percent reactive", names=c("D: Severe","E: Non-Severe"))# of reactive antigens per sample
...
boxplot(mean.intensity ~ Group, data= pData(eset), main= "Intensity of Response", ylab= "Mean Intensity", names=c("D: Severe","E: Non-Severe"))# of reactive antigens per sample
...
```{r, eval=TRUE, fig.align="center", fig.height=4, fig.width=8}
...
with(limmares, plot(logFC, -log10(p.value),cex=.7, pch=20,
                    main = "no complicada vs severa",
                    #col=cols,
                    xlim=c(-2,2), ylim=c(0,15),
                    xlab="log Fold Change"))
abline(h=-log10(0.05),#,-log10(0.001)
       v=c(-1,1),
       lty=2, col= "grey55")
...
#with(subset(limmares, p.value<.05 ), points(logFC, -log10(p.value), pch=19, col="blue"))
#with(subset(limmares, abs(AveExpr)>2), points(logFC, -log10(p.value), pch=20, col="green"))
#with(subset(limmares, abs(AveExpr)>2.40), points(logFC, -log10(p.value), pch=20, col="orange"))
...
#with(subset(limmares, abs(AveExpr)>2), points(logFC, AveExpr, pch=19, col="green"))
#with(subset(limmares, abs(AveExpr)>2.40), points(logFC, AveExpr, pch=19, col="orange"))
...
results <- mutate(limmares, sig=ifelse(limmares$p.value<.0154, "diff rve","below Top10"))
...
p_rep<-p+geom_text_repel(data=filter(results, p.value<.0154), aes(label=Gene.ID))#+xlim=c(-1.5,1.5)
...
resultsq <- mutate(limmares, sigq=ifelse(limmares$AveExpr>2.40, "highly rve","below Top10"))
...
q_rep<-q+geom_text_repel(data=filter(resultsq, AveExpr>2.40), aes(label=Gene.ID))#+xlim=c(-1.5,1.5)
...
boxplot(breadth.percentage ~ con_exp, data= pData(eset), main= "Breadth of Response", ylab= "Percent reactive")
...
boxplot(mean.intensity ~ con_exp, data= pData(eset), main= "Intensity of Response", ylab= "Mean Intensity")
...
ed <- topTable(fit, coef = 2,sort.by ="P", genelist = fit$genes$Description, number = 30)
...
par(mfrow=c(1,2))
with(limmares, plot(logFC, -log10(p.value),cex=.7, pch=20,
                    main = "CON vs SIN episodio previo",
                    #col=cols,
                    xlim=c(-2,2), ylim=c(0,15),
                    xlab="log Fold Change"))
abline(h=-log10(0.05),#,-log10(0.001)
...
results <- mutate(limmares, sig=ifelse(limmares$B>0, "diff rve","below Top10"))
...
#p+geom_text_repel(data=filter(results, p.value<.0224), aes(label=Gene.ID))+xlim(-1,1)
p_rep<-p+geom_text_repel(data=filter(results, B>0), aes(label=Gene.ID))#+xlim=c(-1.5,1.5)
...
**Differentially reactive features**
*using `NMF` package*[@Gaujoux2010NMF]
```{r, eval=TRUE, fig.height=8, fig.width=10, fig.align="center"}
#
#eset<-normal.log2.raw.eSet.VIVAX.SEV.FILTER
pData(eset) <- pData(eset)[,9:ncol(pData(eset))]
...
#library(NMF)
aheatmap(exprs(eset)[rownames(subset(limmares[order(limmares$B, decreasing = TRUE),], B>0 & padj<0.001)),], 
         Rowv = NA, Colv = NA, annCol = pData(eset), layout = "_")
aheatmap(exprs(eset)[rownames(subset(limmares[order(limmares$B, decreasing = TRUE),], B>0 & padj<0.001)),], 
         Rowv = TRUE, Colv = TRUE, annCol = pData(eset), layout = "_")
```
...
```{r, fig.height=6, fig.width=8, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE}
#library(tidyr)
#library(dplyr)
...
#12
corrEXPcon1 <- corrEXP %>%
  gather(feature, reactivity, dplyr::contains("PVX_")) %>%
  mutate(feature=factor(feature)) %>%
...
```{r, fig.height=6, fig.width=9, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE}
#
##### CAMBIAR A BOXPLOT POR CATEGORIAS
#
...
```{r, fig.height=2, fig.width=6, fig.align='center', warning=FALSE, message=FALSE, eval=FALSE}
#
##### CAMBIAR A BOXPLOT POR CATEGORIAS
#
#3
...
```{r, eval=TRUE}
plasmoDBdiff <- read.csv("data-raw/EXPOSURE_DiffRve_REDUCED.csv")
...
23 exp+no-severe, 22 no-exp+severe
...
7 no-exp+no-severe, 22 no-exp+severe
...
#library(NMF)
aheatmap(exprs(eset)[rownames(subset(limmares[order(limmares$AveExpr, decreasing = TRUE),], abs(AveExpr)>2.40)),], 
         Rowv = NA, Colv = NA, annCol = pData(eset), layout = "_")
aheatmap(exprs(eset)[rownames(subset(limmares[order(limmares$AveExpr, decreasing = TRUE),], abs(AveExpr)>2.40)),], 
         Rowv = TRUE, Colv = TRUE, annCol = pData(eset), layout = "_")
```
...
### PlasmoDB protein data
```{r, eval=TRUE}
plasmoDBhigh <- read.csv("data-raw/HIGHLY_Rve_REDUCED.csv")
#colnames(plasmoDBdiff)
...
**between Symptomatology and Highly** 
...
The differentially reactive protein segment w.r.t. exposure is different from the highly reactive protein segment
```{r}
kable(
  rbind(
    #subset(td,td$ID=="major blood-stage surface antigen Pv200"),
    subset(ed,ed$ID=="major blood-stage surface antigen Pv200"),
    #subset(t,t$ID=="major blood-stage surface antigen Pv200"),
    t[2,]), 
  align = "c")
```

**Heatmap of joined features from exposure differentialy and highly reactive subsets**

- *Row sorted by adj.p.val; Columns sorted by Exposure*
- *Row clustered; Columns sorted by Exposure*
- *Row clustered; Columns clustered*
```{r, eval=TRUE, echo=TRUE, fig.height=8, fig.width=10, fig.align="center"}
########################### SAMPLE ORDER per group
...
eset <- eset[feature_sort_mean,sample_sort_EXPOSURE] ## REORDER samples and features inside the ExpressionSet
#library(NMF)
edt <- union(rownames(t),rownames(ed))
aheatmap(exprs(eset)[edt,], Rowv = NA, Colv = NA, annCol = pData(eset), layout = "_")
aheatmap(exprs(eset)[edt,], Rowv = TRUE, Colv = TRUE, annCol = pData(eset), layout = "_")
```
...
- Exposure
    + 30 differencially reactive peptides according to exposure in P.vivax
