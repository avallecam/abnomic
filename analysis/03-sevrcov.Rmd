...
- ¿qué hacer con pacientes con puntaje final de `glasgow` y auscencia 
de puntaje para sus sub-componentes (motor, verbal, ojos)?
- ¿`presenta_comorb` distinta a `enf_cronica`? ¿Cómo están definidas?
- ¿es relevante que incluya `presencia de comorbilidades` o `enfermedades crónicas` 
en la descripción de la muestra?
- ¿se puede asumir un valor de `episodios-previos` con la ayuda de las covariables
asociadas a exposición? ¿vale la pena corregirlo?
...
-------------------------------------------------------------
 Dataset   Pros                      Cons
---------- ------------------------- -------------------------
  nov2014   posee valores             __mayor cantidad de
            cuantitativos originales  NA's__
            de los __criterios de
            laboratorio__:
            glucosa,
            hemoglobina,
            creatinina,
  ...
  sep2016   menor cantidad de NA's    difiere en algunas 
            o __mayor cantidad de     variables, incluido
...
- ¿Las diferencias entre `nov2014` y `sep2016` deben ser consideradas 
como __correcciones__ o __actualizaciones__?
    + [evaluar](#evaluate-covariates) la presencia de diferencias y 
...
- __criterios de laboratorio__ extraídas de `nov2014`.
- variables epidemiológicas y __criterios clínicos__ corregidos en base a `sep2016`.
...
sevdb_pre_1 <- sevdb_pre %>% 
  ...
  mutate(shock= as.double(shock),
         ...
         malaria_pasado_num= as.integer(malaria_pasado_num),
...
tmp <- gplots::venn(list(sevdb_pre=g1_idl, sevdb_pos=g2_idl))
```
```{r}
...
#sevdb_1 %>% select(codigo, temp_local, tiempoenlocal)
```

### Evaluate covariates

* Se evalará:
    - __diferencias__ entre las variables pre y pos actualización
    - el número de elementos __no-NA's__

* Se generarán nuevas [variables consenso](#show-consensus-covariates) para:
    - llenar los vacios en `nov2014`, y
    - corregir la data en `nov2014` por la presente en `sep2016`.
...
sevdb_1 %>% 
  ...
  select(tiempo_local, tiempo_local_1, tiempo_local_2) %>% summarise_all(funs(sum(!is.na(.))))
sevdb_1 %>% 
  mutate(tiempo_local= tiempo_local_1==tiempo_local_2) %>%
...
sevdb_2 <- sevdb_1oms %>% 
  ...
  select(1:3,
         ...
         presenta_comorb, enf_cronica, enf_sum, parasitemia,
         tiempo_local, malaria_pasado_num#,casecontrol
         ) %>% #glimpse()
...
latex(s1, caption='WHO classification: Clinical and Laboratory criterias',
...
sevdb_2 %>% 
  select(codigo, starts_with("sev_WHO"), ends_with("_oms"), convulsion) %>% 
  ...
  select(4:11) %>% summarise_all(sum,na.rm=T)

sevdb_2 %>% 
  select(codigo, sev_WHO, shock_oms) %>% 
  inner_join(sevdb_pre_1 %>% 
               select(codigo, casecontrol
                      , pa_sistolica # solo dos pacientes reclasificados poseen esta variable definida
                      ) %>% 
...
__Elección de los 30 más severos:__

- 14 con `sev_WHO_num`>1
- 26 con `sev_WHO_num`>0 & `shock_oms`>0
- 09 con `sev_WHO_num`>0 & `pulmonar_oms`>0
- __16 con `sev_WHO_num`>0 & `hiperbil_oms`>0__
    + `hiperbil_oms` fue el criterio más importante en la 
    reclasificación de pacientes (Smith & N6, 2013)
- ¿cómo priorizar entre criterios para la virtual elección de 
pacientes con un solo criterio y completar una muestra final de 30?
    + auscencia de __comorbilidades__ y menor frecuencia de __enfermedades crónicas__ de pacientes
    + `pulmonar_oms` sobre `shock_oms` por ser la suma de `edema`+`sdra`?
...
sevdb_30 <- sevdb_2 %>% 
  dplyr::filter(sev_WHO_num>1 | sev_WHO_num>0 & enf_sum<1 & presenta_comorb!=1
                  ...
                  & (hiperbil_oms>0 | pulmonar_oms>0)
                  )
sevdb_30 %>% select(codigo, sev_WHO_num, enf_cronica, presenta_comorb
                    ,hiperbil_oms
                    #,shock_oms
                    ,pulmonar_oms
...
```{r}
# output
sevdb_3 %>% saveRDS("data/03-sevrcov.rds")
```
...
```{r}
sevdb_3 %>% 
  select(codigo, starts_with("sev_WHO"), shock_oms) %>% 
  inner_join(sevdb_pos_1 %>% 
               select(codigo, pa_sistolica) %>% 
               mutate(codigo= str_replace(codigo,"LIM0","LIM")),
             by="codigo") %>% 
...
- excluye a 8 pacientes con dos o más criterios
- el resto son 13 pacientes con un solo criterio
...
- observaciones:
    + a diferencia de la muestra inicial (n=228), 
...
sevdb_1 %>% 
  mutate(tiempo_local= tiempo_local_1==tiempo_local_2) %>%
  select(tiempo_local, tiempo_local_1, tiempo_local_2)%>%
  tidyr::unite(tiempo_local_c, tiempo_local_1, tiempo_local_2, remove = F) %>% #count()
  mutate(tiempo_local_c=str_replace(tiempo_local_c,"(\\d+)_\\1", "\\1")) %>%
  mutate(tiempo_local_c=str_replace(tiempo_local_c,"NA_(\\d+)", "\\1")) %>% 
  mutate(tiempo_local_c=str_replace(tiempo_local_c,"(\\d+)_NA", "\\1")) %>% 
