...
- criterios citados en @WHO2014severe:
...
- criterios citados en @howes2016global:
...
- __muestra total vs sub-muestra__
    - en comparación con una selección de 37 casos severos rerpesentativos 
...
sevdb_2 <- sevdb_1oms %>% 
  ...
  mutate(#sev_ALL_num= rowSums(.[c(18,19)],na.rm = T),
         ...
         episodio_previo=if_else(episodio_previo_num>0,"con","sin")
         ) %>% 
  ...
  mutate(sexo= fct_recode(sexo, "masculino"="1", "femenino"="0")) %>% 
  #relevel(sexo, ref = "female") %>% 
  mutate(sexo= fct_relevel(sexo, "masculino"),
         episodio_previo= fct_relevel(episodio_previo, "sin")) %>% 
  glimpse()
```
...
```{r}
sevdb_3 %>% 
  select(codigo,episodio_previo) %>% 
  filter(is.na(episodio_previo)) %>% 
  dplyr::inner_join(sevdb_pre %>% 
                      dplyr::rename(episodio_previo_num=malaria_pasado_num)%>% 
                      select(codigo,episodio_previo_num,
...
c <- sevdb_2 %>% 
  ...
  geom_histogram(
    #aes(y=..density..), 
                 ...
                 alpha=.5 ,position = "identity") + 
  labs(title="Histograma de Edad",
       subtitle="filter no-severe samples") +
  #geom_density() #+
  scale_x_continuous(breaks=seq(0, 85, by = 5))
...
a <- sevdb_1oms %>%
  select(trombocitopenia, cont_plaquetas) %>% #dplyr::count()
  mutate(trombo_sev= if_else(cont_plaquetas<50000,1,0)) %>% 
  mutate(trombo_sev= as.factor(trombo_sev)) %>% #glimpse()
  ggplot(aes(x=cont_plaquetas, fill=trombo_sev)) + 
  geom_histogram(alpha=.5, position = "identity") + 
...
b <- sevdb_1oms %>%
  select(codigo, trombocitopenia, cont_plaquetas) %>% #dplyr::count()
  mutate(trombo_sev= if_else(cont_plaquetas<50000,1,0)) %>% 
  mutate(trombo_sev= as.factor(trombo_sev)) %>% #glimpse()
  inner_join(sevdb_2 %>% 
               select(codigo, sev_WHO),
