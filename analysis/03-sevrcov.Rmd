...
- **HIPERBILIRRUBINA**
    ...
    + `bilirrubina_1` con decimales completos y `bilirrubina_2` con decimales aproximados a dos dígitos.
    + __OJO:__ `bilitot` posee menos NA's

```{r}
#sevdb_1 %>% 
#  mutate(bilirrubina= bilirrubina_1==bilirrubina_2) %>% #dplyr::count(bilirrubina) #%>% View()
#  filter(bilirrubina==FALSE) %>% 
#  select(bilirrubina, bilirrubina_1, bilirrubina_2, bilitot)
sevdb_1 %>% 
  mutate(bilirrubina= bilirrubina_1==bilirrubina_2) %>%
  select(bilitot, #bilirrubina, 
         bilirrubina_1, bilirrubina_2) %>% summarise_all(funs(sum(!is.na(.))))
sevdb_1 %>% 
  mutate(bilirrubina= bilirrubina_1==bilitot) %>% #dplyr::count(bilirrubina) #%>% View()
  filter(bilirrubina==FALSE) %>% 
  select(bilirrubina, bilitot, bilirrubina_1, bilirrubina_2)
sevdb_1 %>% 
  mutate(bilirrubina= bilirrubina_1==bilitot) %>%
  select(bilirrubina, bilitot, bilirrubina_1, bilirrubina_2) #%>% summarise_all(funs(sum(!is.na(.))))
```
...
sevdb_1 %>% 
  mutate(bilirrubina= bilirrubina_1==bilitot) %>%
  select(bilirrubina, bilirrubina_1, bilitot, bilirrubina_2) %>%
  tidyr::unite(bilirrubina_c, bilirrubina_1, bilitot, remove=F, sep = "_") %>% #count()
...
sevdb_1oms %>% group_by(glas_9, glas_11) %>% dplyr::count()
```
...
sevdb_1oms %>% group_by(#snc_oms, snc_sum, 
                        postracion, #confusion, 
                        coma, convulsion
                        #, snc_normal
                        ) %>% dplyr::count()
```
