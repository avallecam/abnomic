...
j %>% 
  union_all(k) %>% 
  writexl::write_xlsx("table/12-tab04-severe_compare-subject_response.xlsx")
```

```{r}
# simple

glm(formula = sample_freq ~ sev_WHO,data = fin,
...
glm(formula = sample_freq ~ sev_WHO + episodio_previo,data = fin,
    family = gaussian(link = "identity")) %>% 
  epitidy::epi_tidymodel_coef()

glm(formula = sample_mean ~ sev_WHO + episodio_previo,data = fin,
    family = gaussian(link = "identity")) %>% 
  epitidy::epi_tidymodel_coef()
```
