...
td %>% 
  ...
  # view()
  writexl::write_xlsx(
    paste0("table/12-tab03-severe_compare-pvalue_results-np_",
...
sev_t %>% 
  ...
  select(ID,Gene.ID,product_description,everything()) %>% 
  writexl::write_xlsx("table/12-tab06-severe_compare-top_reactives.xlsx")
```
...
bh_pvalues_beween <- 
  paste0(round(pvalue_adj_min,digits = 3), 
         "-", 
         round(pvalue_adj_max,digits = 2))
...
eset_clean_diff_top_sub %>% 
  ...
  labs(y = NULL, x=NULL,
       caption = paste0("*BH adjusted p-values between ",
                        bh_pvalues_beween, 
                        ",\n and absolute log2 fold-change >0.5")) + 
...
j <- fin %>% 
  mutate(sev_WHO=fct_relevel(sev_WHO,"severo")) %>% 
  # count(sev_WHO)
...
fin %>% 
  compareGroups(formula = sev_WHO ~ sample_freq + sample_mean,
                data = .) %>% 
  createTable(sd.type = 2,digits = 2) %>% 
  export2xls("table/12-tab05-severe_compare-subject_response.xls")

j %>% 
  union_all(k) %>% 
  writexl::write_xlsx(
    "table/12-tab04-severe_compare-subject_response.xlsx")
```
...
fin %>% 
  filter(episodio_previo=="sin") %>% 
  filter(!(sample=="LIM2071")) %>% #remove because outlier
  compareGroups(formula = sev_WHO ~ sample_freq + sample_mean,
                data = .) %>% 
  createTable(sd.type = 2,digits = 2)

fin %>% 
  filter(episodio_previo=="con") %>% 
  filter(!(sample=="LIM2071")) %>% #remove because outlier
  compareGroups(formula = sev_WHO ~ sample_freq + sample_mean,
                data = .) %>% 
  createTable(sd.type = 2,digits = 2)
...
# solution:
# t.test and purrr::map
# https://robertamezquita.github.io/post/2017-05-23-using-map-with-generic-functions-like-t-test/

# aim:
# generate a tidy long dataset
# without outliers
ab_subject <- fin %>% 
  mutate(sev_WHO=fct_relevel(sev_WHO,"severo")) %>% 
  mutate(episodio_previo=fct_relevel(episodio_previo,"sin")) %>% 
  # count(episodio_previo)
  filter(!(sample=="LIM2071")) %>% #remove because outlier
  select(sev_WHO,episodio_previo,sample_freq,sample_mean) %>% 
  pivot_longer(cols = sample_freq:sample_mean,
               names_to = "ab_variable",
               values_to = "ab_value")

ab_subject %>% count(ab_variable,sev_WHO)
ab_subject %>% count(ab_variable,sev_WHO,episodio_previo)
ab_subject %>% 
  filter(!is.na(episodio_previo)) %>%
  group_by(ab_variable,episodio_previo) %>% 
  skimr::skim()
ab_subject %>% 
  group_by(ab_variable,sev_WHO) %>% 
  skimr::skim()
ab_subject %>% 
  group_by(ab_variable,sev_WHO,episodio_previo) %>% 
  skimr::skim()

### whole ---
# aim: reject Ho of equal variance
ab_subject %>% 
  group_by(ab_variable) %>% 
  nest() %>% 
  mutate(vtest=map(.x = data,
                   .f = stats:::var.test.formula,
                   formula=formula("ab_value ~ sev_WHO")
                   )) %>% 
  mutate(tidy_vtest=map(.x = vtest,.f = broom::tidy)) %>% 
  select(ab_variable,tidy_vtest) %>% 
  unnest(cols = tidy_vtest) %>% 
  ungroup() %>% 
  select(ab_variable,p.value) %>% 
  identity()

# aim: measurement of dispersion
ab_subject_whole_sd <- ab_subject %>% 
  group_by(ab_variable,sev_WHO) %>% 
  summarise(sd=sd(ab_value)) %>% 
  pivot_wider(id_cols = ab_variable,
              names_from = sev_WHO,
              names_glue = "{sev_WHO}_sd",
              values_from = sd)

# aim: measurement of location and test
ab_subject_whole <- ab_subject %>% 
  group_by(ab_variable) %>% 
  nest() %>% 
  mutate(ttest=map(.x = data,
                   .f = stats:::t.test.formula,
                   formula=formula("ab_value ~ sev_WHO"),
                   var.equal=TRUE
                   )) %>% 
  # pull(ttest)
  mutate(tidy_ttest=map(.x = ttest,.f = broom::tidy)) %>% 
  select(ab_variable,tidy_ttest) %>% 
  unnest(cols = tidy_ttest) %>% 
  ungroup() %>% 
  select(ab_variable,
         "no_severo"=estimate2,
         "severo"=estimate1,p.value,
         estimate,conf.low,conf.high) %>% 
  mutate(analysis="complete") %>% 
  select(analysis,everything()) %>% 
  # view()
  identity() %>% 
  left_join(ab_subject_whole_sd)

### stratified

# aim
# generate nested data set 
# without missings
ab_subject_nested <- ab_subject %>% 
  filter(!is.na(episodio_previo)) %>%
  group_by(ab_variable,episodio_previo) %>% 
  nest() %>% 
  select(ab_variable,episodio_previo,data) %>% 
  arrange(ab_variable) 
...
ab_subject_nested %>% 
  ...
  mutate(tidy_vtest=map(.x = vtest,.f = broom::tidy)) %>% 
  select(ab_variable,episodio_previo,tidy_vtest) %>% 
  unnest(cols = tidy_vtest) %>% 
  ungroup() %>% 
  select(ab_variable,episodio_previo,p.value) %>% 
  identity()

# aim: measurement of dispersion
ab_subject_stratas_sd <- ab_subject %>% 
  filter(!is.na(episodio_previo)) %>%
  group_by(ab_variable,episodio_previo,sev_WHO) %>% 
  summarise(sd=sd(ab_value)) %>% 
  pivot_wider(id_cols = c(ab_variable,episodio_previo),
              names_from = sev_WHO,
              names_glue = "{sev_WHO}_sd",
              values_from = sd)

# aim: measurement of location and test
ab_subject_stratas <- ab_subject_nested %>% 
  mutate(ttest=map(.x = data,
                   .f = stats:::t.test.formula,
                   formula=formula("ab_value ~ sev_WHO"),
                   var.equal=TRUE
  ...
  # pull(ttest)
  mutate(tidy_ttest=map(.x = ttest,.f = broom::tidy)) %>% 
  select(ab_variable,episodio_previo,tidy_ttest) %>% 
  unnest(cols = tidy_ttest) %>% 
  ungroup() %>% 
  select(ab_variable,episodio_previo,
         "no_severo"=estimate2,
         "severo"=estimate1,p.value,
         estimate,conf.low,conf.high) %>% 
  mutate(analysis="stratified") %>% 
  select(analysis,everything()) %>% 
  # view()
  identity() %>% 
  left_join(ab_subject_stratas_sd)

#### one table
ab_subject_table <- 
  ab_subject_stratas %>% 
  bind_rows(ab_subject_whole) %>% 
  arrange(ab_variable,analysis,episodio_previo) %>% 
  select(analysis,ab_variable,episodio_previo,
         starts_with("no_severo"),starts_with("severo"),everything())

ab_subject_table %>% 
  writexl::write_xlsx(
    "table/12-tab07-severe_compare-subject_response-stratified.xlsx"
    )

# to use in stratified plot
ab_subject_table_value <- ab_subject_table %>% 
  select(analysis:episodio_previo,p.value) %>% 
  filter(!is.na(episodio_previo))
```
...
# fin %>% 
#   # filter(!(sample=="LIM2071")) %>% 
#   glm(formula = sample_freq ~ sev_WHO,data = .,
#       family = gaussian(link = "identity")) %>% 
#   # broom::tidy()
#   epitidy::epi_tidymodel_coef()
...
glm(formula = sample_mean ~ sev_WHO,data = fin,
    family = gaussian(link = "identity")) %>% 
  epitidy::epi_tidymodel_coef()

# fin %>% 
#   filter(!(sample=="LIM2071")) %>% 
#   glm(formula = sample_mean ~ sev_WHO,data = .,
#       family = gaussian(link = "identity")) %>% 
#   # broom::tidy()
#   epitidy::epi_tidymodel_coef()

# multiple with confounding variable 

# fin %>% 
#   # filter(!(sample=="LIM2071")) %>% 
#   glm(formula = sample_freq ~ sev_WHO + expo_CAT,data = .,
#       family = gaussian(link = "identity")) %>% 
#   epitidy::epi_tidymodel_coef()
...
glm(formula = sample_mean ~ sev_WHO + expo_CAT,data = fin,
    family = gaussian(link = "identity")) %>% 
  epitidy::epi_tidymodel_coef()

# fin %>% 
#   filter(!(sample=="LIM2071")) %>% 
#   glm(formula = sample_mean ~ sev_WHO + expo_CAT,data = .,
#       family = gaussian(link = "identity")) %>% 
#   epitidy::epi_tidymodel_coef()
```

```{r,eval=FALSE}
# test different presentation of stratified data
# y:     outcome
# x:     exposure
# color: confounder to stratify
# 
# remove outlier for a better test
# filter(!(sample=="LIM2071")) %>% 
fin %>% 
  ggplot(aes(x = sev_WHO, 
             y = sample_freq)) +
  geom_violin(alpha=0,lwd=0.4,draw_quantiles = c(0.25,0.75)) +
  geom_violin(alpha=0,lwd=0.8,draw_quantiles = c(0.5)) +
  ggbeeswarm::geom_quasirandom(method = "tukeyDense",alpha=0.8)

fin %>% 
  filter(!is.na(expo_CAT)) %>%
  mutate(episodio_previo=fct_relevel(episodio_previo,"sin")) %>% 
  ggplot(aes(x = sev_WHO, 
             y = sample_freq)) +
  geom_violin(alpha=0,lwd=0.4,draw_quantiles = c(0.25,0.75)) +
  geom_violin(alpha=0,lwd=0.8,draw_quantiles = c(0.5)) +
  ggbeeswarm::geom_quasirandom(aes(color = episodio_previo), 
                               method = "tukeyDense",alpha=0.8)
...
fin %>% 
  mutate(episodio_previo=fct_relevel(episodio_previo,"sin")) %>% 
  filter(!is.na(expo_CAT)) %>%
  ggplot(aes(x = episodio_previo, #expo_CAT,
             y = sample_freq,
             color = sev_WHO)) +
  geom_violin(alpha=0,lwd=0.4,draw_quantiles = c(0.25,0.75)) +
  geom_violin(alpha=0,lwd=0.8,draw_quantiles = c(0.5)) +
  ggbeeswarm::geom_quasirandom(dodge.width=.9,
                               method = "tukeyDense",alpha=0.8) +
  colorspace::scale_color_discrete_qualitative(rev = TRUE)
```

##### [alt plots with strata]

pending: add labels and pvalue

```{r}
ab_subject_freq <- fin %>% 
  filter(!is.na(expo_CAT)) %>%
  filter(!(sample=="LIM2071")) %>% 
  mutate(episodio_previo=
           fct_relevel(episodio_previo,"sin")) %>% 
  ggplot(aes(x = episodio_previo, 
             y = sample_freq,
             color = sev_WHO)) +
  geom_violin(alpha=0,lwd=0.4,draw_quantiles = c(0.25,0.75)) +
  geom_violin(alpha=0,lwd=0.8,draw_quantiles = c(0.5)) +
  ggbeeswarm::geom_quasirandom(
    dodge.width=.8,
    method = "tukeyDense",alpha=0.4) +
  colorspace::scale_color_discrete_qualitative(
    rev = TRUE,
    labels = c("Non-severe","Severe")) +
  ggsignif::geom_signif(
    annotation = formatC(ab_subject_table_value %>% 
                           filter(ab_variable=="sample_freq",
                                  episodio_previo=="con") %>% 
                           pull(p.value), 
                         digits = 2) %>% 
      paste0("p=",.),
    textsize = 3.3,
    y_position = 330, xmin = 1.8, xmax = 2.2,
  ...
  ggsignif::geom_signif(
    annotation = formatC(ab_subject_table_value %>% 
      ...
      paste0("p=",.),
    textsize = 3.3,
    y_position = 330, xmin = 0.8, xmax = 1.2,
    tip_length = c(0.01, 0.01),
    color="black"
  ) +
  ylim(0,345) +
  scale_x_discrete(labels = c("Without","With")) +
  xlab("Previous Malaria Episodes") + 
...
ab_subject_mean <- fin %>% 
  ...
  ggsignif::geom_signif(
    annotation = formatC(ab_subject_table_value %>% 
                           filter(ab_variable=="sample_mean",
                                  episodio_previo=="con") %>% 
                           pull(p.value), 
                         digits = 2) %>% 
      paste0("p=",.),
    textsize = 3.3,
    y_position = 2.05, xmin = 1.8, xmax = 2.2,
    tip_length = c(0.01, 0.01),
    color="black"
  ) +
  ggsignif::geom_signif(
    annotation = formatC(ab_subject_table_value %>% 
      ...
      paste0("p=",.),
    textsize = 3.3,
    y_position = 2.05, xmin = 0.8, xmax = 1.2,
    tip_length = c(0.01, 0.01),
    color="black"
  ) +
  ylim(-0.2,2.15) +
  scale_x_discrete(labels = c("Without","With")) +
  xlab("Previous Malaria Episodes") + 
  ylab("Median Antigen Reactivity") +
  labs(color = "P. vivax\nmalaria")
...
ab_subject_freq +
  ...
  plot_annotation(tag_levels = "A")
ggsave(
  "figure/04-fig06-severe_compare-subject_response-stratified.png",
       height = 3,width = 6.2,dpi = "retina")
```
