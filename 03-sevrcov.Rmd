...
sevdb_pre_1 <- sevdb_pre %>% 
  dplyr::select(codigo,
         ...
         presenta_comorb:enf_otra_esp, -ends_with("_esp"), -contains("otra"),enf_otra,
         parasitemia, 
         glucosa:casecontrol
         ) %>% 
...
sevdb_pos_1 <- sevdb_pos %>% 
  dplyr::select(codigo,
         ...
         -hiperpirexia, -temperatura, -fcardiaca, frespiratoria,
         malaria_pasado_num, #malaria_pasado_num:tx_malaria,
         presenta_comorb:enf_otra_esp,tx_par_intest, -ends_with("_esp"), -contains("otra"),enf_otra#,
         #parasitemia, 
         #glucosa:casecontrol
         ) %>% #glimpse()
...
#  mutate(bilirrubina_c= as.numeric(bilirrubina_c)) %>%
  ...
  tidyr::unite(enf_otra_c, enf_otra_1, enf_otra_2) %>% #count()
  mutate(enf_otra_c=str_replace(enf_otra_c,"(\\d+)_\\1", "\\1")) %>%
  mutate(enf_otra_c=str_replace(enf_otra_c,"NA_(\\d+)", "\\1")) %>% 
  mutate(enf_otra_c=str_replace(enf_otra_c,"(\\d+)_NA", "\\1")) %>% 
  mutate(enf_otra_c=str_replace(enf_otra_c,"(\\d+)_(\\d+)", "\\2")) %>% #count() #preferencia a _2
  mutate(enf_otra_c=str_replace(enf_otra_c,"NA", replacement = NA_character_)) %>%  #%>% count()
  mutate(enf_otra_c= as.numeric(enf_otra_c)) %>%
  ...
  dplyr::select(
    ...
    parasitemia,tx_par_intest,coag_normal,casecontrol
  ) #%>% glimpse()
```
...
sevdb_1oms <- sevdb_1c %>% 
  ...
  mutate(shock_oms_pa= if_else(pa_sistolica<=80,1,0),# OR shock==1
         ...
         bilsum= select(.,bilind,bildirect) %>% rowSums(na.rm = T),
         hiperbil_oms= if_else(bilsum>2.5,1,0),
         enf_sum= select(.,diabetes,enf_snc,enf_cardiac,enf_pulm,enf_renal,enf_otra) %>% rowSums(na.rm = T),#omite enf_otra
         enf_cronica= if_else(enf_sum>0,1,0),
         sangrado_sum= select(.,gingivorragia,epistaxis,sang_intest) %>% rowSums(na.rm = T),
         sangr_oms= if_else(sangrado_sum>0,1,0)
         #hiperbil_omsX= if_else(bilitot>2.5,1,0),
         #hiperbil_test= hiperbil_omsX==hiperbil_oms,
         ) %>% #glimpse() 
    mutate(
      shock_oms= select(.,shock,shock_oms_pa) %>% rowSums(na.rm = T)
      ) %>% #%>% glimpse()# OR shock==1
  mutate(trombo_sev= if_else(cont_plaquetas<50000,1,0),
         trombo_new= if_else(cont_plaquetas<150000,1,0))
```
...
sevdb_1oms %>%
  ...
  dplyr::count(trombo_new,trombo_sev)
sevdb_1oms %>%
  group_by(trombo_new,trombo_sev) %>%  #glimpse()
  summarise_at(.vars = c("cont_plaquetas"),.funs = c("min","max"))
...
# COMORBIDITIES
sevdb_1oms %>% 
  group_by(presenta_comorb, enf_cronica, diabetes, enf_cardiac, enf_pulm, enf_renal, enf_snc, enf_otra) %>% 
  dplyr::count()
```
...
sevdb_2 <- sevdb_1oms %>% 
  ...
  dplyr::select(1:3,
         anemia_oms,
         hiperbil_oms, hipoglic_oms,
         pulmonar_oms, #sdra, #snc_sum, #snc_oms
         shock_oms, 
         glas_oms, #a.k.a coma
         renal_oms,
         convulsion,
         sangr_oms,trombo_sev,#trombo_new
  ...
  mutate(sev_WHO_num= select(.,anemia_oms:convulsion) %>% rowSums(na.rm = T),#,sev_SNC_num= rowSums(.[11:13],na.rm = T)
         sev_WHO_num_1= select(.,anemia_oms:sangr_oms) %>% rowSums(na.rm = T),
         sev_WHO_num_2= select(.,anemia_oms:trombo_sev) %>% rowSums(na.rm = T),
         sev_WHO_num_3= select(.,anemia_oms:convulsion,trombo_sev) %>% rowSums(na.rm = T),
         sev_WHO_num_4= select(.,anemia_oms:trombo_sev,-convulsion) %>% rowSums(na.rm = T)#,
         ) %>% #glimpse()
  ...
  mutate(#sev_ALL_num= rowSums(.[c(18,19)],na.rm = T),
         ...
         sev_WHO_1= if_else(sev_WHO_num_1>0,"severo","no_severo"),
         sev_WHO_2= if_else(sev_WHO_num_2>0,"severo","no_severo"),
         sev_WHO_3= if_else(sev_WHO_num_3>0,"severo","no_severo"),
         sev_WHO_4= if_else(sev_WHO_num_4>0,"severo","no_severo"),
         episodio_previo=if_else(episodio_previo_num>0,"con","sin")# OJO: AQUI TRADUCCION!!!!
         ) %>% 
  mutate(sexo= as.factor(sexo),
         sev_WHO= as.factor(sev_WHO),
         sev_WHO_1= as.factor(sev_WHO_1),
         sev_WHO_2= as.factor(sev_WHO_2),
         sev_WHO_3= as.factor(sev_WHO_3),
         sev_WHO_4= as.factor(sev_WHO_4),
...
sevdb_2 %>% 
  ...
  dplyr::select(anemia_oms:convulsion) %>% summarise_all(sum,na.rm=T)
...
sevdb_2ws <- sevdb_2 %>% 
  ...
  mutate(Study=if_else(is.na(Study),"whole-db",Study),# %>% as.factor(),
         ...
         sev_WHO_num_1=as.factor(sev_WHO_num_1),
         sev_WHO_num_2=as.factor(sev_WHO_num_2),
         sev_WHO_num_3=as.factor(sev_WHO_num_3),
         sev_WHO_num_4=as.factor(sev_WHO_num_4),
         ...
         sangr_oms=as.factor(sangr_oms),
         #trombo_new=as.factor(trombo_new),
         trombo_sev=as.factor(trombo_sev),
  ...
  mutate(Study=forcats::fct_relevel(Study,"whole-db"),
         parasitemia_mil=parasitemia/1000) %>% 
  mutate(en_zona_endemica_4 = cut(en_zona_endemica,c(0,6,15,26,72)) %>% as.factor(),
         edad_4 = cut(edad,c(4,22,32,52,88)) %>% as.factor(),
         parasitemia_mil_4 = cut(parasitemia_mil,c(0,1.330,3.338,6.038,42.683)) %>% as.factor()
         )
...
sevdb_3sv <- sevdb_3 %>% 
  mutate(casecontrol=as.factor(casecontrol),
         sev_WHO_num=as.factor(sev_WHO_num),
         sev_WHO_num_1=as.factor(sev_WHO_num_1),
         sev_WHO_num_2=as.factor(sev_WHO_num_2),
         sev_WHO_num_3=as.factor(sev_WHO_num_3),
         ...
         enf_cronica=as.factor(enf_cronica),
         parasitemia_mil=parasitemia/1000,
         en_zona_endemica_4 = cut(en_zona_endemica,c(0,6,15,26,72)) %>% as.factor(),
         edad_4 = cut(edad,c(4,22,32,52,88)) %>% as.factor(),
         parasitemia_mil_4 = cut(parasitemia_mil,c(0,1.330,3.338,6.038,42.683)) %>% as.factor()) %>% 
...
data_s <- sevdb_2ws %>% 
  mutate(edad_10=cut(edad,c(quantile(edad,probs = seq(0,1,0.1),na.rm = T)[1],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[2],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[3],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[4],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[5],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[6],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[7],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[8],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[9],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[10],
                                       quantile(edad,probs = seq(0,1,0.1),na.rm = T)[11]),
                           include.lowest = T),
         edad_4=cut(edad,c(quantile(edad,probs = seq(0,1,0.25),na.rm = T)[1],
                                       quantile(edad,probs = seq(0,1,0.25),na.rm = T)[2],
                                       quantile(edad,probs = seq(0,1,0.25),na.rm = T)[3],
                                       quantile(edad,probs = seq(0,1,0.25),na.rm = T)[4],
                                       quantile(edad,probs = seq(0,1,0.25),na.rm = T)[5]),
                           include.lowest = T)) 

#data_s %>% dplyr::count(edad)
data_s %>% dplyr::count(edad_4)

data_s %>% filter(!is.na(edad)) %>% 
  ggplot(aes(edad_4)) +
...
tab_ws <- compareGroups(Study ~ 
                          edad + sexo +
                          en_zona_endemica +
                          enf_cronica + #presenta_comorb + 
                          #tx_par_intest + 
                          parasitemia_mil + 
                          episodio_previo + #episodio_previo_num + 
                          sev_WHO_2 #+ #sev_WHO_num +
                ,
              data = sevdb_2ws, byrow=F, method=c(edad=2,en_zona_endemica=2,parasitemia_mil=2)
              ) 
tab_ws %>% createTable(show.all = T,show.p.overall = F)
tab_ws %>% createTable(show.all = T,show.p.overall = F) %>% export2xls("table/tab1-in_out.xls")
```
...
tab_cr %>% createTable(show.all = T,show.n = F,show.p.overall = F,hide.no = "0") #%>% export2xls("table/tab1.xls")
tab_cr %>% createTable(show.all = T,show.n = F,show.p.overall = F,hide.no = "0") %>% export2xls("table/tab2-in_out_oms.xls")
```

```{r,eval=FALSE}
sevdb_2ws %>% 
  filter(sev_WHO_num==1 & Study == "in-microarray") %>% 
  select(anemia_oms:trombo_sev,sev_WHO,sev_WHO_2)

hist(sevdb_2ws$en_zona_endemica)
summary(sevdb_2ws$en_zona_endemica)
hist(sevdb_2ws$edad)
summary(sevdb_2ws$edad)
hist(sevdb_2ws$parasitemia_mil)
summary(sevdb_2ws$parasitemia_mil)
hist(sevdb_2ws$parasitemia)
summary(sevdb_2ws$parasitemia)
```

### table 3: whole counfunders

```{r}
#hist(sevdb_2ws$episodio_previo_num)
#sevdb_2ws %>% dplyr::count(episodio_previo_num)
tab_sv <- compareGroups(sev_WHO_2 ~ 
                          edad + #edad_4 + 
                          sexo +
                          ...
                          #sev_WHO #+ #sev_WHO_num +
                ,
              data = sevdb_2ws, byrow=T, method=c(edad=2,en_zona_endemica=2,episodio_previo_num=2,parasitemia_mil=2)
              ) 
tab_sv %>% createTable(show.all = T,show.n = F) #%>% export2xls("table/tab3-all-severe.xls")
tab_sv %>% createTable(show.all = T,show.n = F) %>% export2xls("table/tab3-all-severe.xls")
...
tab_sv <- compareGroups(sev_WHO_2 ~ 
                          ...
                          #sev_WHO #+ #sev_WHO_num +
                ,
              data = sevdb_3sv, byrow=T, method=c(edad=2,en_zona_endemica=2,episodio_previo_num=2,parasitemia_mil=2)
              ) 
tab_sv %>% createTable(show.all = T,show.n = F) #%>% export2xls("table/tab1.xls")
tab_sv %>% createTable(show.all = T,show.n = F) %>% export2xls("table/tab3-in_severe.xls")
```
